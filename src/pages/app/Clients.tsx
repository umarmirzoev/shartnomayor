  ...
  return (
    <div className="max-w-6xl">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-bold text-ink-950">{t.app.clients.title}</h1>
          <p className="mt-1 text-sm text-ink-500">{clients.length} {t.app.clients.countSuffix}</p>
        </div>
            ...
            >
              ...
              <span
                className={clsx(
                  ...
                  status === tab.value ? 'bg-white/20 text-white' : 'bg-ink-100 text-ink-500'
                )}
              >
                {counts[tab.value]}
              </span>
      ...
      <Card className="overflow-hidden">
        <div className="hidden grid-cols-[1fr_110px_120px_1fr_80px_40px] gap-4 border-b border-ink-100 bg-ink-50/60 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-ink-400 sm:grid">
        ...
        <div className="divide-y divide-ink-100">
          {filtered.map((c) => (
            <Link
              key={c.id}
              to={`/app/clients/${c.id}`}
              className="grid grid-cols-1 gap-2 px-5 py-4 transition hover:bg-ink-50 sm:grid-cols-[1fr_110px_120px_1fr_80px_40px] sm:items-center sm:gap-4"
            >
          ...
          {filtered.length === 0 && <p className="px-5 py-8 text-center text-sm text-ink-400">{t.common.notFoundShort}</p>}
        </div>
      </Card>

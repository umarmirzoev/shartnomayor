          ...
          <div className="grid grid-cols-[190px_1fr]">
            <div className="hidden border-r border-ink-100 bg-ink-50/60 p-4 sm:block">
              <div className="mb-4 flex items-center gap-2 px-1">
                ...
                <span className="text-xs font-bold text-ink-900">ШартномаЁр</span>
              </div>
              <div className="space-y-1">
                {[
                  ...
                  { label: 'Клиенты' },
                  { label: 'Дела' },
                  { label: 'Новый черновик', accent: true },
                  { label: 'Шаблоны' },
                ].map((i) => (
            ...
            <div className="p-4 sm:p-5">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="text-[13px] font-bold text-ink-900">Добро пожаловать, Фарход</p>
                  <p className="text-[11px] text-ink-400">7 из 20 ИИ-запросов использовано в этом месяце</p>
                </div>
                <span className="flex items-center gap-1 rounded-full bg-gold-100 px-2.5 py-1 text-[10px] font-bold text-gold-700"><Sparkles size={10} /> ИИ активен</span>
              </div>
              <div className="mb-3 grid grid-cols-3 gap-2.5">
                {[
                  { icon: FileText, label: 'Черновики', value: '4' },
                  { icon: Users, label: 'Клиенты', value: '5' },
                  { icon: Bell, label: 'Уведомления', value: '2' },
                ].map((s) => (
              ...
              <div className="rounded-xl border border-ink-100 bg-white p-3">
                <p className="mb-2 text-[11px] font-bold text-ink-700">Последние черновики</p>
                {[
                  { title: 'NDA — Помир Технолоджис', status: 'Готов', icon: CheckCircle2, tone: 'text-emerald-500' },
                  { title: 'Аренда офиса — пр. Рудаки', status: 'На проверке', icon: Clock, tone: 'text-amber-500' },
                ].map((d) => (
